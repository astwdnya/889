"""
babestube_handler.py
────────────────────
استخراج و دانلود ویدیو از babestube.com (با پشتیبانی چند کیفیت)

روش کار (بر اساس تحلیل واقعی صفحه با analyze_babestube.py):
  - سایت پشت Cloudflare CDN هست ولی challenge فقط در HTML هست (محتوای اصلی هم موجوده)
  - aiohttp مستقیم کار می‌کنه (سرور HTTP 200 با محتوای کامل برمی‌گردونه)
  - Player: KT Player (مثل cartoonporn، fetishshrine، hihentaiporn)
  - URL ویدیو از /get_file/1/HASH/.../FILEID.mp4/?v-acctoken=TOKEN میان
    (نکته: babestube از /get_file/1/ استفاده می‌کنه، نه /get_file/3/)
  ...
  - yt-dlp با extractor=generic کار می‌کنه و هر دو کیفیت رو پیدا می‌کنه

چند کیفیت (مهم):
  KT Player از flashvars استفاده می‌کنه:
    - video_url + video_url_text: '480p'         → کیفیت پایه (SD)
    - video_alt_url + video_alt_url_text: '720p'  → کیفیت HD
    - video_alt_url_hd: '1'                        → flag که می‌گه HD موجوده

  اگه کیفیت‌های دیگه هم باشن، این الگو رو دنبال می‌کنن:
    - video_alt_url2 + video_alt_url2_text: '1080p'
    - video_alt_url2_hd: '1'

استراتژی دانلود:
  1. fetch صفحه با aiohttp (سریع — سایت CF challenge real نداره)
  2. fallback به curl_cffi با impersonate=chrome
  3. استخراج video_url و video_alt_url از flashvars block
  4. fallback به regex برای پیدا کردن URL های /get_file/ با v-acctoken
  5. multi-segment download با 16 workers (سرور Range رو پشتیبانی می‌کنه)
...
logger = logging.getLogger("BabeStubHandler")
...
def is_babestube_url(url: str) -> bool:
    """بررسی اینکه URL مربوط به babestube هست."""
    try:
        host = urlparse(url).hostname or ""
        return host in _ALLOWED_HOSTS or host.endswith(".babestube.com")
    except Exception:
        return False
...
def _extract_title(html: str) -> str:
    """استخراج عنوان ویدیو."""
    # روش 1: از flashvars (بهترین — title کامل با ' رو هم درست برمی‌گردونه)
    flashvars = _extract_flashvars(html)
    if flashvars.get("video_title"):
        title = flashvars["video_title"].strip()
        # حذف " @ BabesTube.com" یا " - BabesTube.com"
        title = re.sub(
            r"\s*[@|-]\s*BabesTube\.com\s*$",
...
def _extract_flashvars(html: str) -> dict:
    ...
    # نکته: title ممکنه ' داشته باشه (مثل "Let's") که regex ساده رو خراب می‌کنه
    # پس اول title رو جداگانه استخراج می‌کنیم
    pairs = []
    
    # الگوی special برای video_title (که ممکنه ' داشته باشه)
    # فرمت: video_title: '...', (یا پایان block با })
    ...
    for k, v in pairs:
        ...
        # در صورت وجود escape های JS
        if k == "video_title":
            v_decoded = v_decoded.replace("\\'", "'")
...
def _extract_video_sources(html: str) -> List[dict]:
    ...
    اولویت 1: video_url و video_alt_url از flashvars (KT Player)
              — این مهم‌ترین منه چون quality_text هم داره
    ...
    Returns:
        ...
        مرتب شده بر اساس کیفیت (بالاترین اول)
    """
    sources = []
    seen_urls = set()

    # ─── روش 1: از flashvars (مهم‌ترین — کیفیت‌ها رو مشخص می‌کنه) ───
    flashvars = _extract_flashvars(html)

    # کیفیت پایه (video_url + video_url_text)
    if flashvars.get("video_url"):
        url = _clean_url(flashvars["video_url"])
        if _is_main_video_url(url) and url not in seen_urls:
            seen_urls.add(url)
            quality_text = flashvars.get("video_url_text", "default")
            height = _quality_text_to_height(quality_text)
            quality_key = quality_text.lower() if quality_text else "default"
            label = f"📺 MP4 {quality_text}" if quality_text else "📺 MP4 (default)"
            sources.append({
                "label": label,
                "url": url,
                "height": height,
    ...
    # کیفیت‌های alt (video_alt_url + video_alt_url_text)
    # الگو: video_alt_url, video_alt_url2, video_alt_url3, ...
    alt_keys_patterns = [
        # (url_key, text_key, hd_flag_key)
        ...
        ("video_alt_url5", "video_alt_url5_text", "video_alt_url5_hd"),
    ]
    ...
    for url_key, text_key, hd_key in alt_keys_patterns:
        ...
        height = _quality_text_to_height(quality_text) if quality_text else 720
        quality_key = quality_text.lower() if quality_text else "alt"
        label = f"📺 MP4 {quality_text}" if quality_text else "📺 MP4 (alt)"
        ...
        logger.info("Found %s from flashvars (%s, HD=%s): %s", url_key, quality_text, is_hd, url[:100])

    # ─── روش 2: پیدا کردن URL های با v-acctoken (از HTML) ───
    # فرمت: https://www.babestube.com/get_file/1/HASH/22000/22677/22677_720p.mp4/?v-acctoken=TOKEN
    ...
    for m in vacctoken_pattern.finditer(html):
        ...
        if "_1080p" in url_lower:
            ...
            label = "📺 MP4 (default)"
            height = 480  # default معمولا 480p هست
            quality_key = "default"
            is_hd = False
...
def _quality_text_to_height(quality_text: str) -> int:
    """تبدیل quality_text ('480p', '720p', 'HD', 'SD') به height عددی."""
    if not quality_text:
        return 480
...
async def _fetch_page(
    ...
    """
    # ── روش 1: aiohttp ──
    # (babestube CF challenge real نداره — فقط در HTML marker هست ولی محتوای اصلی هم موجوده)
    try:
        ...
        async with aiohttp.ClientSession(
            ...
            async with session.get(url, allow_redirects=True) as resp:
                if resp.status == 200:
                    ...
                    # چک کن که video_url وجود داره (یعنی محتوای واقعی هست)
                    if "video_url" in html or "flashvars" in html:
                        logger.info("Page fetched via aiohttp, size=%d", len(html))
                        return html, local_jar, ""
                    else:
                        logger.warning("aiohttp: 200 ولی video_url پیدا نشد — احتمالا CF challenge")
...
async def extract_babestube_qualities(
    url: str,
    ...
    Returns:
        ...
        qualities: list of dicts with keys: label, url, height, quality_key, method, is_hd
                   مرتب شده بر اساس height (بالاترین اول)
        title: str
        info: dict with extra info (thumbnail, cookies, etc.)
    """
    if not is_babestube_url(url):
        return [], "Invalid URL", {}
...
async def _download_multi_segment(
    ...
    از aiohttp استفاده می‌کنه چون سایت CF challenge real نداره.
    """
    try:
        ...
        logger.info(f"[DL-BABES] Work-queue: {total_chunks} chunks, {num_workers} workers, total={content_length}")
            ...
            while True:
                ...
                for attempt in range(max_retries):
                    ...
                    try:
                        ...
                        logger.warning(f"[DL-BABES] W{worker_id} c{c_idx} attempt {attempt+1} failed: {e}")
        ...
        try:
            ...
            if worker_failures or failed_chunks:
                logger.warning(f"[DL-BABES] {len(worker_failures)} workers failed, {len(failed_chunks)} chunks failed")
                _cleanup_file(filepath)
                return False, f"Multi-segment failed: {len(failed_chunks)} chunks", 0

        except Exception as e:
            logger.error(f"[DL-BABES] Work-queue error: {e}", exc_info=True)
            try:
                await shared_session.close()
            except Exception:
                pass
        ...
        if file_size != content_length:
            logger.warning(f"[DL-BABES] Size mismatch: expected={content_length}, got={file_size}")

        elapsed = time.time() - start_time
        avg_speed = file_size / elapsed / 1024 / 1024 if elapsed > 0 else 0
        logger.info(f"[DL-BABES] Multi-segment DONE | size={_format_size(file_size)} | time={elapsed:.1f}s | avg_speed={avg_speed:.1f} MB/s")
        return True, "", file_size

    except Exception as e:
        logger.error(f"[DL-BABES] Multi-segment error: {e}", exc_info=True)
        _cleanup_file(filepath)
        return False, str(e)[:200], 0
...
async def _download_with_ytdlp(
    ...
    # format selector — yt-dlp با format_id=quality کار می‌کنه
    ...
    try:
        cmd = [
            ...
            "--add-header", f"Referer:https://babestube.com/",
            "-o", filepath,
        ]
        ...
        logger.info(f"[DL-BABES] yt-dlp DONE | size={_format_size(size)}")
        return True, "", size
        ...
        logger.error(f"[DL-BABES] yt-dlp error: {e}", exc_info=True)
        _cleanup_file(filepath)
        return False, str(e)[:200], 0
...
async def download_babestube_video(
    page_url: str,
    video_url: str,
    filepath: str,
    progress_cb: Optional[ProgressCallback] = None,
    ...
    """
    if not is_babestube_url(page_url):
    ...
    # ── روش 1: multi-segment ──
    logger.info("[DL-BABES] Attempt 1: multi-segment")
    ...
    if error == "HTTP_403":
        ...
        logger.info("[DL-BABES] 403, refreshing session...")
        if progress_cb:
        ...
        if jar:
            ...
            # re-extract URL
            qualities, _, info = await extract_babestube_qualities(
                page_url, progress_cb=None
            )
            if qualities:
                # انتخاب همون کیفیت
                video_url = qualities[0]["url"]
                logger.info("[DL-BABES] Got fresh URL with v-acctoken")
        # retry multi-segment
        success, error, size = await _download_multi_segment(
            video_url, filepath, referer, cookies, progress_cb, dl_id=dl_id,
        )
        if success:
            return True, "", size
    logger.info(f"[DL-BABES] Multi-segment failed: {error}")
    _cleanup_file(filepath)

    # ── روش 2: single-connection ──
    logger.info("[DL-BABES] Attempt 2: single-connection")
    success, error, size = await _download_single_aiohttp(
        video_url, filepath, referer, cookies, progress_cb, dl_id=dl_id,
    )
    if success:
        return True, "", size
    logger.info(f"[DL-BABES] Single failed: {error}")
    _cleanup_file(filepath)

    # ── روش 3: yt-dlp ──
    logger.info("[DL-BABES] Attempt 3: yt-dlp on page URL")
...
async def download_babestube_direct(
    url: str,
    filepath: str,
    progress_cb: Optional[ProgressCallback] = None,
    video_url: str = "",
    ...
    Args:
        ...
        quality: 'high', 'low', '720p', '480p', '1080p', '360p', 'default'
    ...
    if not video_url:
        ...
        # انتخاب کیفیت
        selected = None
        # اولویت با quality_key دقیق
        for q in qualities:
            if q.get("quality_key") == quality:
                selected = q
                break
        # fallback: high → بالاترین (HD اول)، low → پایین‌ترین
        if not selected:
    ...
    return await download_babestube_video(
