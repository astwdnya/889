...
def cleanup_expired_sessions() -> int:
    ...
    now = time.time()
    expired = [
        sid for sid, data in teensexvideos_sessions.items()
...
def _extract_title(html: str) -> str:
    ...
    if m:
        ...
        title = re.sub(
            r"\s*[-|]\s*[Tt]een\s*[Ss]ex\s*[Vv]ideos.*$", "", title
        ).strip()
...
async def _download_with_ytdlp(
    ...
    try:
        cmd = [
            ...
            "--add-header", "Referer:https://www.teensexvideos.xxx/",
            "--add-header", "Origin:https://www.teensexvideos.xxx",
